...
const TOP10_ASSETS = [
  'GOLD','EURUSD','GBPUSD','USDJPY','BTCUSD','ETHUSD',
  'Apple','MICROSOFT','US500Cash','US30Cash'
];
...
function checkAll(symbol, candles, balance = DEFAULT_BALANCE, context = {}) {
  try {
    ...
    if (rsi !== null) {
      if (rsi < 30)
        potential.push(makeSignal('COMPRA','rsi',WEIGHTS.rsi,['RSI < 30'],now,symbol));
      if (rsi > 70)
        potential.push(makeSignal('VENDA','rsi',WEIGHTS.rsi,['RSI > 70'],now,symbol));
    }

    const macdSignal = MACD(candles,12,26,9);
    if (macdSignal === 'BUY')
      potential.push(makeSignal('COMPRA','macd',WEIGHTS.macd,['MACD BUY'],now,symbol));
    if (macdSignal === 'SELL')
      potential.push(makeSignal('VENDA','macd',WEIGHTS.macd,['MACD SELL'],now,symbol));

    const boll = BollingerBands(candles,20,2);
    if (boll) {
      if (last.close < boll.lower)
        potential.push(makeSignal('COMPRA','bollinger',WEIGHTS.bollinger,['Close < lower BB'],now,symbol));
      if (last.close > boll.upper)
        potential.push(makeSignal('VENDA','bollinger',WEIGHTS.bollinger,['Close > upper BB'],now,symbol));
    }

    const vol = detectVolumeSpike(candles);
    if (vol)
      potential.push(makeSignal(vol.side === 'up' ? 'COMPRA' : 'VENDA','volume_spike',WEIGHTS.volume_spike,['Volume spike'],now,symbol));
    ...
    if (TOP10_ASSETS.includes(symbol)) {
      ...
      if (scalpCond)
        potential.push(makeSignal(scalpCond.side,'scalp_gold',WEIGHTS.scalp_gold,['Scalp pattern + volume'],now,symbol));

      // Breakout com volume
      const breakout = detectBreakout(candles);
      if (breakout)
        potential.push(makeSignal(breakout.side === 'up' ? 'COMPRA' : 'VENDA','breakout',WEIGHTS.breakout,['Breakout with volume'],now,symbol));
    }
...
// -----------------------------
// Helpers
function makeSignal(side, strategy, weight, reasons, now, symbol) {
  return { side, strategy, weight, reasons, time: now.toISOString(), symbol };
}

function SMA(candles, period) {
  if(candles.length<period) return null;
  return candles.slice(-period).reduce((a,c)=>a+c.close,0)/period;
}
...
function MACD(candles,fast=12,slow=26,signalPeriod=9){
  ...
  const emaFast=EMA(candles,fast), emaSlow=EMA(candles,slow);
  if(emaFast===null||emaSlow===null) return null;
  const macdLine=emaFast-emaSlow;
  const macdSignal=EMA_for_value_series(candles,fast,slow,signalPeriod);
  if(macdSignal===null) return null;
  if(macdLine>macdSignal) return 'BUY';
  if(macdLine<macdSignal) return 'SELL';
  return null;
}

function EMA_for_value_series(candles,fast,slow,signalPeriod){
  if(candles.length<slow+signalPeriod) return null;
  const macdSeries=[];
  for(let i=0;i<candles.length;i++){
    const sub=candles.slice(0,i+1);
    const ef=EMA(sub,fast), es=EMA(sub,slow);
    if(ef!==null && es!==null) macdSeries.push(ef-es);
  }
  if(macdSeries.length<signalPeriod) return null;
  let ema=macdSeries[macdSeries.length-signalPeriod];
  const k=2/(signalPeriod+1);
  for(let i=macdSeries.length-signalPeriod+1;i<macdSeries.length;i++){
    ema=macdSeries[i]*k+ema*(1-k);}
  return ema;
}
...
function detectVolumeSpike(candles){
  ...
  const avg=last10.reduce((a,c)=>a+c.volume,0)/last10.length;
  const last=last10[last10.length-1];
  if(last.volume>avg*1.8){
    const side=last.close>last.open?'up':'down';
    return { side };
  }
  return null;
}
...
function detectBreakout(candles,lookback=20){
  ...
  if(last.close>maxHigh && last.volume>avgVol*1.2) return {side:'up'};
  if(last.close<minLow && last.volume>avgVol*1.2) return {side:'down'};
  return null;
}

function scalpConditionForGold(candles){
  if(candles.length<10) return null;
  const last=candles[candles.length-1], prev=candles[candles.length-2];
  const rsi6=RSI(candles,6);
  if(!rsi6) return null;
  const volSpike=last.volume>prev.volume*1.5;
  if(volSpike && Math.abs(last.close-last.open)/(last.high-last.low+1e-9)>0.6){
    const side=last.close>prev.close?'COMPRA':'VENDA';
    if((side==='COMPRA'&&rsi6<70)||(side==='VENDA'&&rsi6>30)) return { side };
  }
  return null;
}

function touchedEMA(candles,period,lookback=8){
  const emaVal=EMA(candles,period);
  if(!emaVal) return false;
  return candles.slice(-lookback).some(c=>Math.abs(c.close-emaVal)/emaVal<0.0025);
}

function computeHigherTrend(symbol,higher={},baseCandles=[]){
  try{
    const candDaily=higher.daily;
    const candWeekly=higher.weekly;
    const choose=candDaily||candWeekly||null;
    if(!choose||choose.length<50){
      if(baseCandles.length>=200){
        const ema200=EMA(baseCandles,200), ema72=EMA(baseCandles,72);
        if(ema200&&ema72) return { direction: ema72>ema200?'up':'down', ema200, note:'fallback_self_ema' };
      }
      return null;
    }
    const ema200=EMA(choose,200), ema72=EMA(choose,72);
    if(!ema200||!ema72) return null;
    const dir=ema72>ema200?'up':'down';
    return { direction:dir, ema200, note:'higher_tf' };
  }catch(e){return null;}
}
  ...
  try{
    ...
    for(const item of news){
      ...
      if(Math.abs(now-t)<=windowMs){
        ...
        if(TOP10_ASSETS.includes(symbol)&&(title.includes(symbol.toLowerCase())||title.includes('fed')||title.includes('cpi')||title.includes('inflation')))
          return 0.35;
        return 0.10;
      }
    }
...
function applyConfidenceModifiers(signal, newsPenalty, higherTrend, marketStrength, correlationFactor, vix){
  ...
  if(higherTrend && higherTrend.direction){
    if((higherTrend.direction==='up' && signal.side==='VENDA') || (higherTrend.direction==='down' && signal.side==='COMPRA'))
      conf*=0.45;
